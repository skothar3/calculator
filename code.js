...
const regZero = /^-?0$/;
const regMinTwoDigitsOrOneDecimal =
  /^((?!.*e)-?\d{2,})|((?!.*e)-?\d{1,}\.(\d{1,})?)|(-?\d{1}\.?(\d{1,})?e[\+-]\d{1,})$/;
const regMinOneDigit =
  /^((?!.*e)-?\d{1,}\.?(\d{1,})?)|(-?\d{1}\.?(\d{1,})?e[\+-]\d{1,})$/;
const regMinOneDigitAndOperator =
  /^(((?!.*e)-?\d{1,}\.?(\d{1,})?)|(-?\d{1}\.?(\d{1,})?e[\+-]\d{1,}))\s[+-x/\^]$/;
const regStartsWithOneDigit =
  /^((?!.*e)-?\d{1,}\.?(\d{1,})?)|(-?\d{1}\.?(\d{1,})?e[\+-]\d{1,})/;
const regStartsWithNegative = /^\-/;
const regEndsWithOperator = /[+-x/\^]$/;
const regEndsWithEquals = /=$/;
const regContainsDecimal = /\./;
...
document.addEventListener("keydown", (e) => {
  const keyMatch = document.querySelector(`button[data-key-match="${e.key}"]`);
  if (!keyMatch) return;
...
function updateDisplay(btnTarget) {
  ...
  if (
    regMinOneDigit.test(screenDisplay.textContent) &&
    screenDisplay.textContent.length > 7 &&
    !btnTarget.className.includes("operator") &&
    !(
      btnTarget.value == "=" ||
      btnTarget.value == "Clear" ||
      btnTarget.value == "Delete"
    )
  ) {
  ...
  if (
    /(Infinity|NaN)/.test(screenDisplay.textContent) ||
    /(Infinity|NaN)/.test(historyDisplay.textContent)
  ) {
  ...
  if (btnTarget.value == "Delete") {
    if (regZero.test(screenDisplay.textContent)) return;
    if (regMinTwoDigitsOrOneDecimal.test(screenDisplay.textContent)) {
  ...
  if (regZero.test(screenDisplay.textContent) && !historyDisplay.textContent) {
    if (btnTarget.className.includes("number")) {
      ...
      btnTarget.className.includes("special") &&
      btnTarget.value == "."
    ) {
  ...
  // If a operator key is pressed => Case 3
    ...
    regMinOneDigitAndOperator.test(historyDisplay.textContent) &&
    !screenDisplay.textContent
  ) {
    if (btnTarget.className.includes("number")) {
      screenDisplay.textContent = btnTarget.value;
    } else if (btnTarget.className.includes("operator")) {
      historyDisplay.textContent = historyDisplay.textContent.replace(
        /[+-x/\^]$/,
        btnTarget.value
      );
    } else if (
  ...
  // If a operator key is pressed => Case 3
    ...
    regEndsWithEquals.test(historyDisplay.textContent) &&
    screenDisplay.textContent
  ) {
    if (btnTarget.className.includes("number")) {
      historyDisplay.textContent = "";
      ...
      historyDisplay.textContent = `Sqrt ${screenDisplay.textContent} =`;
      screenDisplay.textContent = operate(
        "Sqrt",
        screenDisplay.textContent,
        null
      );
